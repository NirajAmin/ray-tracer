...
void cornell_with_subsurface_scattering()
{
    hittable_list world;
    ...
    world.add(make_shared<quad>(point3(555, 0, 555), vec3(-555, 0, 0), vec3(0, 555, 0), white)); // back wall

    // Light source
    world.add(make_shared<quad>(point3(213, 554, 227), vec3(130, 0, 0), vec3(0, 0, 105), light));

    // White box in the scene
    shared_ptr<hittable> box1 = box(point3(0, 0, 0), point3(165, 330, 165), white);
    box1 = make_shared<rotate_y>(box1, 15);
    box1 = make_shared<translate>(box1, vec3(265, 0, 295));
    world.add(box1);
    ...
    auto red_wax = make_shared<lambertian>(
        color(0.8, 0.1, 0.1),
        1.0f,
        0.5f);
    world.add(make_shared<sphere>(point3(370, 100, 270), 100, red_wax));
...
int main()
{
    cornell_with_subsurface_scattering();
}