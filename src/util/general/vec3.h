...
public:
    ...
    CUDA_HOST_DEVICE vec3() : e{0, 0, 0} {}
    CUDA_HOST_DEVICE vec3(double e0, double e1, double e2) : e{e0, e1, e2} {}

    CUDA_HOST_DEVICE double x() const { return e[0]; }
    CUDA_HOST_DEVICE double y() const { return e[1]; }
    CUDA_HOST_DEVICE double z() const { return e[2]; }

    CUDA_HOST_DEVICE vec3 operator-() const { return vec3(-e[0], -e[1], -e[2]); }
    CUDA_HOST_DEVICE double operator[](int i) const { return e[i]; }
    CUDA_HOST_DEVICE double &operator[](int i) { return e[i]; }
    ...
    CUDA_HOST_DEVICE double length_squared() const
    {
...
#include <curand_kernel.h>
    ...
    CUDA_HOST_DEVICE static vec3 random(double min, double max)
    {
...
CUDA_HOST_DEVICE inline std::ostream &operator<<(std::ostream &out, const vec3 &v)
{
...
CUDA_HOST_DEVICE inline vec3 operator*(double t, const vec3 &v)
{
    return vec3(t * v.e[0], t * v.e[1], t * v.e[2]);
}

CUDA_HOST_DEVICE inline vec3 operator*(const vec3 &v, double t)
{
    return t * v;
}
...
CUDA_HOST_DEVICE inline double dot(const vec3 &u, const vec3 &v)
{
...
CUDA_HOST_DEVICE inline vec3 random_unit_vector()
{
    while (true)
    {
